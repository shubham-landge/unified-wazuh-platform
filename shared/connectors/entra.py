...
class EntraConnector:
    ...
    async def _acquire_token(self) -> str | None:
        if not self.client_id or not self.client_secret:
            logger.warning("Entra connector: client_id/client_secret not configured")
            return None
        ...
        try:
            ...
            logger.error("Entra token acquisition failed: %s", exc)
            return None
    ...
    async def health(self) -> dict:
        ...
        headers = await self._auth_headers()
        if not headers:
            return {"connected": False, "error": "Entra not configured", "latency_ms": 0}
        try:
            async with httpx.AsyncClient(timeout=20.0) as client:
